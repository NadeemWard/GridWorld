...
class Transitions_Probs:
    ...
    def __init__(self,grid,actions, data = None):
        ...
        :param actions: the actions of the environment. Have it be a list.
        ...
        self.grid = grid
        ''' create a 3D matrix of size |S|x|S|x|A| '''
        self.t_probs = np.zeros((len(grid.states),len(actions),len(grid.states)) )
    ...
    def create_common_transition(self,common_type):
        ...
        self.t_probs = np.zeros((len(self.grid.states), len(self.actions), len(self.grid.states)))

        if(common_type == "Deterministic"):

            '''
            This considers all entries to be 0.
            '''
